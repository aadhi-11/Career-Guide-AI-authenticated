...
export default function ChatSessionPage() {
  ...
  // Single useEffect for authentication and error handling
  useEffect(() => {
    if (isLoaded && !user) {
      router.push("/");
    }
  ...
  const { data: session, isLoading: sessionLoading, error: sessionError } = trpc.getSession.useQuery(
    { sessionId },
    { 
      enabled: isLoaded && !!user && !!sessionId,
      retry: false,
    }
  );
  ...
  // Scroll to bottom function - memoized to prevent unnecessary re-renders
  ...
  // Memoized message deduplication - prevents unnecessary re-renders
  ...
  }, [inputMessage, isLoading, session, sessionId, addUserMessageMutation, updateTitleMutation, addMessageMutation, utils, scrollToBottom]);
  ...
  // Memoized message status indicator component
  const MessageStatusIndicator = useCallback(({ messageId }: { messageId: string }) => {
